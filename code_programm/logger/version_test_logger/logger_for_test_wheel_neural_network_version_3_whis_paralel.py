...
def update_wheel_position_from_queue(gamepad, queue):
    while True:
        # Блокирующий вызов до тех пор, пока не появятся новые значения в очереди
        positions = queue.get()
        for position in positions:
            gamepad.left_joystick_float(x_value_float=position, y_value_float=0.0)
            gamepad.update()
...
def interpolation_wheel_position(new_array_wheel_position, position_gamepad_value, array_wheel_pos):
    if abs(new_array_wheel_position[-1] - position_gamepad_value[1].item()) > 0.01:
        delta = 1.1
    elif abs(new_array_wheel_position[-1] - position_gamepad_value[1].item()) > 0.0001:
        delta = 1.4
    else:
        delta = 1.7
    array_wheel_position = []
    for t in array_wheel_pos:
        interpolated_value = linear_interpolation(new_array_wheel_position[-1], position_gamepad_value[1].item(),
                                                  t / delta)
...
def main():
    ...
    array_wheel_pos = [i / 100 for i in range(0, 55)]
    ...
    update_thread = threading.Thread(target=update_wheel_position_from_queue, args=(gamepad, position_queue),
                                     daemon=True)
    update_thread.start()
    ...
    try:
        while True:
            if recording:
                array_wheel_position = [joystick.get_axis(0)] * 30
                ...
                while opened:
                    ...
                    position_queue.put(array_wheel_position)
