...
from code_programm.wheel_neural_network.lstm.ver_5.wheel_neural_network_lstm_5 import LSTMModel
...
def model_speed_processing(model_speed, speed_image):
    ...
    if sorted_objects:
        ...
        speed = torch.tensor([int(speed)/170], device='cuda')
    else:
        speed = torch.tensor([int(30)/170], device='cuda')
    return speed


def update_wheel_position_from_queue(virtual_wheel, queue_speed_pos):
    while True:
...
def main_code():
    ...
    update_position = Thread(target=update_wheel_position_from_queue,
                             args=(virtual_wheel, queue_speed_pos),
                             daemon=True)
        ...
        while True:
            if recording:
                ...
                while opened:
                    ...
                    new_tensor_wheel_position = model_wheel.forward(combined_tensor.unsqueeze(0).unsqueeze(0))
