...
from code_programm.wheel_neural_network.forward.ver_2.wheel_neural_network_forward import FeedforwardNet
...
def get_weight_model():
    ...
    wheel_net.load_state_dict(torch.load(get_path_weight_model('weight_wheel_nn_version_2.pth')))
...
def new_position_gamepad(gamepad, new_wheel_position):
        ...
        while True:
            if recording:
                ...
                while opened:
                    ...
                    new_position_gamepad(gamepad, array_wheel_position[0])

                    road_img, speed_img = save_photo(camera, road_area, speed_area)

                    new_position_gamepad(gamepad, array_wheel_position[1])
                    ...
                    new_position_gamepad(gamepad, array_wheel_position[2])

                    speed = model_speed_predict(model_speed, speed_img)

                    new_position_gamepad(gamepad, array_wheel_position[3])

                    new_position_gamepad(gamepad, array_wheel_position[4])

                    tensor_wheel_position = model_wheel.forward(combined_tensor, speed)
